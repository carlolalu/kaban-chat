...
impl Message {
    ...
    fn from_paket(candidate: Vec<u8>) -> Result<Message, MsgFromUtf8PaketError> {
...
pub mod test_util {
    ...
    pub fn craft_random_msg(username: &str) -> Result<Message, TextValidityError> {
        let valid_text = craft_random_valid_text(Message::MAX_CONTENT_LEN);
        Message::new(username, &valid_text)
    }
}
...
pub mod test {
    ...
    fn test_craft_random_msg() {
        for _ in 0..100 {
            ...
            let random_msg_result = test_util::craft_random_msg(&random_username);

            // println!("msg_result: [[[{:?}]]]", random_msg_result);

            assert_eq!(random_msg_result.is_ok(), true);
        }
    }
    ...
    fn test_msg2paket2msg() {
        ...
        let valid_msg = Message::new(&valid_name, &valid_content).unwrap();
        let valid_paket = valid_msg.clone().paket().unwrap();

        assert_eq!(Message::from_paket(valid_paket), Ok(valid_msg));

        let random_invalid_string_as_bytes: Vec<u8> = format!("aaaa")
            .as_bytes()
            .into_iter()
            .map(|&byte| byte)
            .collect();
    ...
    async fn test_message_depaketer() {
        ...
        task_tracker.spawn(async move {
            message_depaketer(reader, tx).await.unwrap();
