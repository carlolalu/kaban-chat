...
pub mod test {
    ...
    fn test_msg2paket2msg() {
        ...
        let valid_msg = Message::new(&valid_name, &valid_content).expect("The message is not valid");
        let valid_paket = valid_msg.clone().paket().expect("The paket is not valid");

        assert_eq!(Message::from_paket(valid_paket), Ok(valid_msg));

        let random_invalid_string_as_bytes: Vec<u8> = "aaaa"
            .as_bytes()
            .into_iter()
            .map(|&byte| byte)
            .collect();
    ...
    async fn test_message_depaketer() {
        ...
        let paket: Vec<u8> = (0..num_messages)
            .map(|num| {
                ...
                random_message.paket().expect("The paketing of a random message failed.")
            })
            .flatten()
            .collect();
        ...
        task_tracker.spawn(async move {
            message_depaketer(reader, tx).await.expect("The message depaketer failed.");
